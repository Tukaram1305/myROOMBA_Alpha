...
//    SIDEBRUSH  BRUSH  BLOWER  LIGHTS  CurrMOTOR  BATTERY    GEAR    BLCK/InvRev  SonarL  SonarR(teraz TEMP BAT)
byte LCD_DATA[10] =   {0,0,0,0,0,0,3,0,0,13};
byte LCD_DATA_p[10] = {0,0,0,0,0,0,3,0,0,13};
...
void loop() {
  ...
  if (LCD_DATA_p[4] != LCD_DATA[4] || ONCE==true) // CurrMOTOR
  {
    ...
    */
    ...
    switch(LCD_DATA[4]) {
      case 0: { cmFrame.draw(20, 110, 120, 125, TFT_WHITE); break; } // ALL
      case 1: { cmFrame.draw(20, 110, 40, 125, TFT_RED); break; }    // SBRUSH
      case 2: { cmFrame.draw(60, 110, 40, 125, TFT_RED); break; }    // BRUSH
      case 3: { cmFrame.draw(100, 110, 40, 125, TFT_RED); break; }   // BLOWER
      default: break; }
  ...
  if ( BatIncomFLAG == true ) // BATTERY
  {
    ...
    */
    ...
    else 
    {
      ...
      else bPc = map(BATMED, 105, 130, 1, 99);
  
      int original = (BATMED)*4;
      BATV = 5./1023*original * 6.46; // 6.46 -> coef z dzielnika nap.
      BATTEMP = float(LCD_DATA[9])/2.f;
      tft.setTextColor(BLACK); tft.setTextSize(2);
      tft.setCursor(65, 50);
      mess = "B "+String(bPc_p)+"% "+String(BATVp,1)+"V "+String(BATTEMPp,1)+"*C";
      tft.print(mess);
      tft.setTextColor(YELLOW); tft.setTextSize(2);
      tft.setCursor(65, 50);
      mess = "B "+String(bPc)+"% "+String(BATV,1)+"V "+String(BATTEMP,1)+"*C";
      tft.print(mess);
