...
// v.1.4 - dodanie samoskrecania przy L/R blokadzie, dod. twarde hamowanie
// Radio NRF
#include <SPI.h>
#include <nRF24L01.h>
#include <RF24.h>
...
const uint16_t BRUSH_MAXPWM  = 1020; // 26% / 25% MAKS 1020
...
void handle_adjPots()
{
  // Pierwszy adj.pot - kalibracja accel
  adjPot_1_val = analogRead(adj_1pot);
  if (adjPot_1_val > adjPot_1_val_p+8 || adjPot_1_val < adjPot_1_val_p-8 )
  { 
    int pSP = map(adjPot_1_val, 0, 1023, -15, 15);
    motors.setPID_SP(pSP);
    //Serial.println("CurrSP: "+String(motors.givePID_SP()));
    adjPot_1_val_p = adjPot_1_val; 
  }

  /* // Drugi adj.pot - chwilowo po nic
...
void loop() {
  ...
  // Lewy POT - Przednie/Tylne swiatla
  L_pot_check();

  //Wysylanie na EKRAN - 5/s chwilowo wystarczajaco
